...
const UserBirthdayCard = ({ user }) => {
  ...
  const validate  = (values) => {
    ...
    if (!values.birthMessage) {
      ...
      errors.birthMessage = 'Поздравление слишком длинное (максимум 100 символов)'
    }
    ...
    <div key={user.id} className="personEvent__card">
      ...
      <div className="personEvent__cardBody">
        <div className="personEvent__personInfo">
          <h2 className="personEvent__name">{user.name}</h2>
        ...
        <div className="personEvent__personCongrats">
          {
            user.commonCongrats
              ? user.commonCongrats.map((congrats) => {
                return (
                  <div key={congrats.id} className="personEvent__eventMessage eventMessage">
                    <span className="eventMessage__name">
                      {congrats.name}
                    </span>
